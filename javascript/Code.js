...
function enviarNotificacionEquipoValidado(requestId, correoValidador, nombreValidador, equipo, estado, observaciones, validationStatus) {
  try {
    const subject = `✅ ${equipo} ha validado - Solicitud #${requestId}`;
    
    let equiposValidadosHTML = '';
    let equiposPendientesHTML = '';
    
    for (const teamName in validationStatus.teamValidationStatus) {
      const teamStatus = validationStatus.teamValidationStatus[teamName];
      if (teamStatus.validated) {
        const validadorPor = teamStatus.validatedBy ? teamStatus.validatedBy.validador : 'N/A';
        equiposValidadosHTML += `<li><strong>${teamName}</strong> - Validado por: ${validadorPor}</li>`;
      } else {
        equiposPendientesHTML += `<li><strong>${teamName}</strong> - Pendiente</li>`;
      }
    }
...
<div style="font-family: Arial, sans-serif; max-width: 650px; margin: auto; padding: 20px;
  ...
  <h2 style="color: #2e7d32; text-align: center; margin-bottom: 20px;">✅ Validación de Equipo Registrada</h2>
  
  <p>El <strong>${equipo}</strong> ha registrado una validación en la solicitud <strong>#${requestId}</strong>.</p>
  
  <div style="background: #e8f5e9; padding: 15px; border-radius: 5px; margin: 15px 0;">
    <p style="margin: 0;"><strong>Validador:</strong> ${nombreValidador} (${correoValidador})</p>
    <p style="margin: 5px 0 0 0;"><strong>Observaciones:</strong> ${observaciones || 'Ninguna'}</p>
  </div>
  
  <h3 style="color: #2e7d32;">📊 Estado de Validación por Equipos</h3>
  
  <div style="display: flex; gap: 20px; margin: 20px 0;">
    <div style="flex: 1; background: #e8f5e9; padding: 15px; border-radius: 5px;">
      <h4 style="margin-top: 0; color: #2e7d32;">✅ Equipos Validados</h4>
      <ul style="padding-left: 20px;">
        ${equiposValidadosHTML || '<li>Ninguno</li>'}
      </ul>
    </div>
    
    <div style="flex: 1; background: #fff3e0; padding: 15px; border-radius: 5px;">
      <h4 style="margin-top: 0; color: #e65100;">⏳ Equipos Pendientes</h4>
      <ul style="padding-left: 20px;">
        ${equiposPendientesHTML || '<li>Ninguno</li>'}
      </ul>
    </div>
  </div>
  ...
  <p style="margin-top: 20px; color: #555;">
    Saludos,<br>
    <strong>Sistema de Baja de Elementos - COMARRICO</strong>
  </p>
</div>
    `;
    ...
    const uniqueEmails = [...new Set(allTeamEmails)];
    
    enviarCorreoConCopia(uniqueEmails.join(','), subject, body);
    console.log(`✅ Notificación de equipo enviada a ${uniqueEmails.length} destinatarios`);
    
  } catch (error) {
    console.error('Error al enviar notificación de equipo:', error);
  }
}
...
function submitAmbientalValidation(requestId, precioReal, cantidadReal, observaciones, validador) {
  try {
    ...
    // Enviar notificación
    enviarNotificacionAmbiental(requestData, precioReal, cantidadReal, observaciones, validador);
...
function enviarNotificacionAmbiental(requestData, precioReal, cantidadReal, observaciones, validador) {
  try {
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (rowId === targetId) {
                validations.push({
                    ...
                    fecha: data[i][9] ? formatDate(data[i][9]) : '',
                    validadoPor: data[i][10] || ''
                });
            }
        }
